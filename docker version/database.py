...
def add_data_mempool(data, node_id, timestamp,in_chain_status = 0 ):
    ...
    cursor = conn.cursor()
    cursor.execute("""
                   INSERT OR IGNORE INTO mempool(hash,
                                       time,
                                       data,
                                       node_id,
                                       in_chain)
...
def query_newest_block():
    ...
    cursor.execute("""
                   ...
                   SELECT block_hash,
                          previous_hash,
                          height, timestamp, difficulty, miner, data, chain_work, nonce
...
def get_active_tip_block_data() -> dict:
    ...
    cursor.execute("""
                   ...
                   FROM blockchain
                   WHERE is_main_chain = 1
                   ORDER BY chain_work DESC LIMIT 1
                   """)
...
def add_new_block(block_data_dict, block_hash, main_chain_value=1):
    ...
    cursor = conn.cursor()
    cursor.execute("""
                   INSERT INTO blockchain(block_hash,
                                          previous_hash,
                                          height,
                                          timestamp,
                                          difficulty,
...
def get_chain_from_tip(tip_hash):
    ...
    cursor.execute("""
                   WITH RECURSIVE chain_path AS (SELECT block_hash,
                                                        previous_hash,
                                                        height
                                                 FROM blockchain
                                                 WHERE block_hash = ?
